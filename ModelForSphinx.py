...
def load_data():
    """
       ...
       Np arrays to store de dataset and divide every pixel to 255.0 result into unic value between 0 and 1.
...
def save_data(images,labels):
    """
       Save the concatenate images with their corresponding labels for using later to test the model in another script.
...
def augment_add(images,seg: iaa.Sequential,labels):
    """
        Apply data augmentation to the images with ship labels.
        In our case we decide to apply two times per image on purpose to have a smooth distribution
        ...
        - augmented_labels: NumPy array, the corresponding augmented labels.
        """
...
def create_model():
    """
        Create a convolutional neural network model.(specific for computer vision aplication)
        ...
        After Flatten starts normal layers with relu as activation function(right for this kind of aplication) and sigmoid for last layer(right for models that use binary classification)        

        Returns:
        - model: keras.models.Sequential, the created model.
        """
    model=Sequential()
...
def first_view(images,labels):
    """
       Visualize the distribution of classes and show a random image.
...
def conca_view(images,labels):
    """
        Visualize the distribution of classes after augmentation.
...
def suffle_set(images,labels):
    """
        Shuffle the images and labels with same seed to keep image from position x correlated to corresponding label
        ...
        - shuffled_labels: NumPy array, the corresponding shuffled labels.
        """
    np.random.seed(42)
...
def check_suffle(iamges,labels):
    """
        Check the shuffled images and labels.
...
def divide_set(images,labels):
    """
        ...
        With a procent of 70% for training data, 20% for validation data and 10% for test data
        ...
        - test_labels: NumPy array, the corresponding labels for testing.
        """
    size=len(images)
    train=int(0.7*size)
...
def distrib_train(train_labels):
    """
        ...
        Args:
        - train_labels: NumPy array, the labels of the training set.
        """
...
def train(model,eps):
    """
        Train the model. Where magic happens.
...
if __name__=='__main__':
    ...
    Aug= True
    seq=iaa.Sequential([iaa.Fliplr(0,5),iaa.LinearContrast((0.75,1.5)),iaa.Multiply((0.8,1.2),per_channel=0.2),iaa.Affine(scale={"x": (0.8, 1.2), "y": (0.8, 1.2)}, rotate=(-25, 25), shear=(-8, 8) )],random_order=True)
